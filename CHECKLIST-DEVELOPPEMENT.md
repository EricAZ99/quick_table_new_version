...
- [x] Implémenter le cache Redis `rbac:resolved:{membershipId}` — 0,5j — nouveau `middlewares/rbacPermissionsCache.ts` (`getCachedPermissions`/`setCachedPermissions`), TTL 10 min (doc 26 §26.2). doc 26 §26.2 documente aussi une invalidation événementielle sur `EmployeeRoleChanged` (doc 20, Event Bus) en complément du TTL — volontairement pas implémentée ici : aucun endpoint de changement de rôle n'existe encore (Feature 2.2, pas commencée), rien n'émet cet événement à ce jour ; TTL seul pour l'instant, à compléter plus tard. `rbac.middleware.ts` refondu autour d'un seul `resolvedPermissions` (fusion rôle+overrides, doc 26 §26.4 "cache de permissions résolues") : lu depuis le cache si présent, sinon recalculé depuis `roleDefinitions` puis mis en cache — remplace le court-circuit séparé du ticket précédent (l'ancien chemin "override seul, sans lire roleDefinitions" n'existe plus : le cache-miss construit toujours l'ensemble combiné complet, c'est la mise en cache qui évite désormais la relecture, pas un raccourci par requête). **Best-effort explicite** (doc 26 §26.6, "le pire cas est un recalcul") : toute erreur Redis (y compris client non initialisé) est capturée et journalisée en `warn`, jamais propagée — la vérification RBAC retombe silencieusement sur une lecture directe de `roleDefinitions`, jamais bloquée par une panne du cache ; validé en pratique pendant ce ticket même, le quota Upstash étant épuisé (voir plus bas). Vérifié réellement : 6 tests unitaires du module cache (hit/miss/erreur Redis en lecture et écriture, get/set jamais d'exception) + suite `rbac.middleware.spec.ts` étendue à 13 tests (dont hit-sans-lecture-DB, miss-avec-écriture-cache, fusion rôle+overrides) + `rbac.middleware.integration.spec.ts` gagne un sous-bloc dédié (`describe.skipIf(!redisUrl)`) connectant un vrai Redis pour vérifier le contenu réel de `rbac:resolved:{membershipId}` et qu'un second appel ne relit plus `roleDefinitions` (spy sur `RoleDefinitionModel.findOne`). **Ce sous-test échoue actuellement** — pas un bug : le quota Upstash toujours épuisé fait échouer silencieusement le `SET` (capturé par le best-effort, `logger.warn` invisible en env de test car niveau `silent`), donc le second appel recalcule ; les 4 tests principaux du même fichier, eux, passent sans connexion Redis du tout (`getRedisClient()` lève "non initialisé", capturé de la même façon) — preuve en conditions réelles que le design best-effort fonctionne : la vérification RBAC reste correcte (200/403 corrects) même Redis totalement indisponible, seule l'optimisation de cache ne peut pas être vérifiée tant que le quota bloque. Suite complète relancée : aucune régression ailleurs (un échec isolé de `tenantIsolationFixtures.integration.spec.ts` observé une fois en run complet a été reproduit comme un flake de charge concurrente, confirmé en repassant au vert isolément, sans lien avec ce ticket). **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).
- [x] Écrire les tests de la matrice de permissions — 1j — deux nouveaux fichiers : `database/seeders/__tests__/roleDefinitions.matrix.spec.ts` (16 tests) retranscrit **indépendamment** toute la matrice doc 08 §8.4 (relue et ré-écrite depuis le document, sans copier `roleDefinitions.seed.ts`, pour avoir une vraie chance d'attraper une divergence) et compare par égalité d'ensemble exacte (ni manquant, ni en trop) contre `ROLE_DEFINITIONS_SEED_DATA` pour chacun des 5 rôles — confirme la transcription du ticket 1 correcte, aucun écart trouvé. `middlewares/__tests__/rbac.permissionMatrix.integration.spec.ts` (20 tests) sweep de bout en bout contre le **vrai** `seedRoleDefinitions()` et le vrai `requirePermissionAsync` (appelé directement, pas de route HTTP — chaque permission de la matrice correspondrait à une route dédiée par construction doc 08 §8.7, en monter des dizaines pour ce seul sweep aurait été un détour inutile) sur un échantillon représentatif par rôle (accordées et refusées) contre un vrai MongoDB Atlas. **Bug réel découvert et corrigé en écrivant ce ticket** (pas un flake, reproduit 2 fois puis corrigé et revérifié vert sur 3 runs complets consécutifs) : `roleDefinitions` est une collection globale, **non tenant-scoped** (doc 08 §8.1) — contrairement à tout le reste du projet, aucune dimension (`tenantId`) ne permet à des fichiers de test de s'isoler les uns des autres dessus. `roleDefinitions.integration.spec.ts` (ticket 1) et `rbac.middleware.integration.spec.ts` (ticket 2) faisaient chacun un `deleteMany({})` destructeur sur la collection entière en `beforeAll`/`afterAll` ; comme Vitest exécute les fichiers de test en parallèle par défaut et que ces trois fichiers tournent contre le même MongoDB Atlas partagé (pas d'isolation par worker), un `deleteMany` d'un fichier pouvait effacer la matrice pendant qu'un autre fichier l'interrogeait — observé concrètement comme `'waiter' → 'tables:change_status'` passant de `granted` à `denied` selon le timing d'exécution. **Décision validée avec toi** ("refactor ciblé, sans wipe destructeur", plutôt que désactiver le parallélisme de toute la suite) : retiré tout `deleteMany({})` sur `RoleDefinitionModel` dans les 3 fichiers concernés — `seedRoleDefinitions()` (idempotent, ticket 1) suffit ; `rbac.middleware.integration.spec.ts` utilise désormais le vrai seed au lieu de `roleDefinitions` fabriqués à la main (jeu de données réconcilié, ses assertions existantes restent valides contre la vraie matrice) ; le test de versionnement de `roleDefinitions.integration.spec.ts` (qui mute délibérément `waiter` pour tester le mécanisme) restaure désormais explicitement l'état d'origine en fin de test et utilise une permission synthétique (`test:versioning_probe`, jamais une vraie permission métier) pour ne plus jamais entrer en collision avec les assertions d'un autre fichier. Risque résiduel accepté et documenté : une fenêtre transitoire, courte et auto-réparée (un seul document, pas toute la collection), reste possible si deux fichiers s'exécutent exactement au même instant — un échec franc (erreur d'index unique) plutôt qu'une corruption silencieuse, jugé acceptable. Suite complète relancée 3 fois : aucune régression, seuls les échecs Redis déjà connus (quota Upstash) subsistent, de façon variable selon le run. **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).
...
- [x] Modéliser/implémenter `restaurants` (CRUD) avec `country`/`locale`/`currency` dérivés — 1j — **incohérence de doc signalée, non corrigée** : le checklist parle d'un "CRUD restaurants" générique, mais doc09 §9.3/§9.4 ne documente ni Create ni Delete côté tenant — seuls `GET/PATCH /restaurants/me`(`/settings`) existent pour un tenant, la création passe exclusivement par `POST /platform/restaurants` (super_admin), qui déclenche le provisioning transactionnel complet de doc06 §6.7 (`restaurants` + **`subscriptions`** + premier `membership` owner) — or `subscriptions` n'est prévu qu'en **Feature 9.1** (Epic 9), pas Epic 2. **Décision validée avec toi** ("provisioning réduit maintenant") : `POST /platform/restaurants` construit ici crée `restaurants` + premier `membership` `restaurant_owner` dans une vraie transaction MongoDB (`mongoose.startSession()`/`withTransaction`, première utilisation de transactions dans le projet) mais **sans** l'étape `subscriptions` (Feature 9.1) ni les données de référence (salle par défaut/catégories, Feature 2.3/3.2) — `ownerId` référence un utilisateur déjà existant, aucune création de compte inline (pas d'endpoint public d'inscription self-service dans doc09 à ce jour, autre gap signalé). Pas de `DELETE` construit (doc06 §6.7 : purge différée, hors périmètre de ce ticket). `country`/`locale`/`timezone`/`currency` tous requis explicitement dans `createRestaurantSchema` pour l'instant — la dérivation automatique depuis `countryDefaults` est le ticket suivant de cette Feature (un ticket à la fois). Schéma complet doc 05 §"restaurants" (`database/models/restaurant.model.ts`) — `_id` sert de `tenantId` partout ailleurs, donc **pas de plugin `tenantScope`** (même raisonnement que `users`) ; `openingHours.day` en jour de semaine explicite et `open`/`close` en `HH:mm` (convention choisie, non détaillée par doc05). `RestaurantsRepository`/`RestaurantsService` (slug auto-généré depuis `name`, suffixe numérique en cas de collision) + `RestaurantsController` (`getMe`/`updateMe`/`updateMeSettings`/`create`) + nouveau router `platformRestaurantsRouter` monté sur `/api/v1/platform/restaurants` (préfixe distinct de `/api/v1/restaurants`, pour que la frontière super_admin/tenant soit visible dans la structure des routes). Nouveaux codes **400 `RESTAURANT_INVALID_PAYLOAD`**, **404 `RESTAURANT_NOT_FOUND`**, **404 `RESTAURANT_OWNER_NOT_FOUND`** (fr/en/it/es). **Refactors transverses au passage** : `requireTenantContext` (extrait de `hello-world.controller.ts` vers `shared/utils/`, 2ᵉ quasi-doublon avec `restaurants.controller.ts` — même précédent que `createRedisRateLimiter`) ; `MembershipsRepository.create` gagne un paramètre `session` optionnel (nécessaire pour participer à la transaction du provisioning réduit). Vérifié réellement : 61 tests (modèle, repository, validators, service, controller — tous unitaires/mockés — + 14 tests d'intégration réels contre MongoDB Atlas : création réelle restaurant+membership, 403 non-super_admin, 404 owner inconnu, 400 payload invalide, 401 sans token, GET/PATCH `/me` et `/me/settings` avec vérification RBAC réelle owner vs waiter, persistance vérifiée en base) ; suite complète relancée, aucune régression (547/591, seuls les 2 échecs Redis déjà connus — quota Upstash — subsistent). **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).
- [x] Implémenter la dérivation automatique devise/langue/fuseau depuis `countryDefaults` — 0,5j — `locale`/`timezone`/`currency` deviennent **optionnels** dans `createRestaurantSchema` (seul `country` reste obligatoire, doc 09 §9.3) ; la dérivation elle-même vit dans `RestaurantsService#deriveLocaleTimezoneCurrency` (jamais dans le schéma Zod — validation de forme vs logique métier restent des couches séparées, doc 12 §12.2). Toute valeur explicite du payload prime toujours sur `countryDefaults` (surcharge partielle possible : ex. `currency` fourni, `locale`/`timezone` dérivés) ; une seule requête `countryDefaults` même si plusieurs champs manquent. Nouveau **422 `RESTAURANT_COUNTRY_DEFAULTS_MISSING`** (`BusinessRuleError`, doc 09 §9.1 — payload syntaxiquement valide mais donnée de référence manquante pour le traiter, distinct d'un 400) si le pays n'a pas d'entrée `countryDefaults` et qu'un champ reste manquant après surcharge. **Bug de collision inter-fichiers trouvé et corrigé au passage** (même classe que le bug `roleDefinitions` déjà corrigé en Feature 1.4, cette fois sur `countryDefaults`) : `countryDefaults.integration.spec.ts` (Feature 0.4) faisait un `deleteMany({})` global en `beforeAll`/`afterAll` sur cette collection non tenant-scoped — les nouveaux tests de dérivation de ce ticket (`restaurants.integration.spec.ts`) la lisent maintenant aussi, exposant la même race que Vitest exécute les fichiers de test en parallèle par défaut. Corrigé en retirant le `deleteMany` global (l'upsert de `seedCountryDefaults()` est déjà idempotent et suffisant) et en faisant nettoyer son doublon de test (`ZZ`) par le test lui-même plutôt que par un `afterAll` global ; le nouveau test "pays sans countryDefaults" de ce ticket utilise délibérément un code pays différent (`YY`) pour ne jamais entrer en collision avec le sentinel `ZZ` de l'autre fichier. Vérifié réellement : 71 tests (dont 17 d'intégration réelle contre MongoDB Atlas — dérivation complète, surcharge partielle, 422 pays inconnu, plus les 14 déjà existants) tous verts ; suite complète relancée, aucune régression (565/599, seuls les échecs Redis déjà connus — quota Upstash — subsistent). **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).
- [x] Écran d'inscription : saisie manuelle du pays ou confirmation de la détection automatique — 1j — **incohérence de doc signalée** : le nom du ticket ("écran d'inscription") suppose un flux d'inscription self-service, mais `POST /platform/restaurants` (seul endpoint de création existant, doc 09 §9.3) est réservé `super_admin` et exige un `ownerId` déjà existant — doc 06 §6.7 évoque le principe d'une inscription "self-service" sans qu'elle soit construite dans la surface d'API réelle. **Décision validée avec toi** ("version autonome minimale") : construit uniquement ce que le ticket décrit littéralement — un composant qui appelle `GET /restaurants/detect-location` (déjà public, existant) et laisse confirmer/corriger le pays, sans Router ni Pinia (aucun des deux n'existe encore dans `apps/web`, encore au stade bootstrap Feature 0.4) ni soumission réelle vers le backend ; le résultat (`{country, countryDetectionMethod}`) est émis via un event Vue (`confirmed`) pour un futur flux d'inscription complet à brancher plus tard. Pays limités aux 5 couverts par `countryDefaults` (au-delà, la création réelle échouerait de toute façon en 422 `RESTAURANT_COUNTRY_DEFAULTS_MISSING`, Feature 2.1 ticket précédent).
      Premiers vrais morceaux d'infra frontend posés au passage (aucun des deux n'existait, `apps/web` étant resté au stade "bootstrap" depuis Feature 0.4) : `assets/tokens.css` (tokens du Design System v1, `docs/design/00-design-system.html`, importé une fois dans `main.ts` plutôt que dupliqué par composant) ; proxy Vite `/api` → `http://localhost:3000` (`vite.config.ts`) pour éviter d'avoir à construire CORS côté API juste pour le dev local (middleware pas encore construit, doc 12 §12.4) ; premier appel `fetch` du frontend (`composables/useCountryDetection.ts`, pas de client HTTP partagé construit pour un seul appel, doc 14 §14.5 KISS — à factoriser au 2ᵉ appel réel). `App.vue` (placeholder "bootstrap OK" depuis Feature 0.4) monte désormais ce premier vrai écran ; `App.spec.ts` réduit à un smoke-test (le détail est couvert par `CountryDetectionScreen.spec.ts`).
      Vérifié réellement : 23 tests (dont 10 pour l'écran couvrant les 3 chemins — pays détecté supporté à confirmer, détection en échec ou pays non supporté → sélection manuelle directe, retour en arrière depuis l'écran confirmé — et 4 pour le composable, dont le cas panne réseau) tous verts ; `pnpm typecheck`/`eslint` propres. **Vérification navigateur partielle** : le serveur de dev API refuse de démarrer (même blocage Redis/Upstash déjà connu, fail-fast au boot, sans lien avec ce ticket) — impossible de vérifier le flux bout-en-bout avec un vrai backend actif. Vérifié à la place : le serveur de dev Vite démarre et sert l'app réellement (HTML, `main.ts`, `App.vue` compilés et servis sans erreur, `CountryDetectionScreen.vue` résolu correctement) et le proxy `/api` intercepte bien les requêtes (500 de connexion refusée plutôt qu'un 404, confirmant le forward vers le port 3000) ; pas de capture d'écran littérale possible (aucun outil de navigateur automatisé disponible dans cet environnement, `chromium-cli` absent, Playwright non installé). **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).
- [x] Écran back-office : création/édition restaurant (horaires, logo, coordonnées) — 1j — **gap signalé** : le ticket a besoin d'un Access Token réel pour `GET/PATCH /restaurants/me` (`requireAuth`), mais aucun écran de connexion n'existe nulle part dans ce checklist et `apps/web` n'avait ni Pinia ni mécanisme de token. **Décision validée avec toi** ("toujours créer le frontend quand un ticket en exige") : construit un vrai flux d'authentification plutôt qu'un stub — `LoginScreen.vue` (email/mot de passe, `POST /auth/login`, gère aussi le challenge 2FA `POST /auth/2fa/verify` si le compte l'a activée) + `stores/auth.store.ts` (Pinia, **introduit ici** — premier état réellement partagé entre composants, `App.vue`/`main.ts` mis à jour en conséquence) qui garde l'Access Token en mémoire uniquement (jamais `localStorage`, doc 07 §7.2) et restaure la session au chargement via `POST /auth/refresh` (cookie `refreshToken` httpOnly). `shared/apiClient.ts` (2ᵉ+ appels réels : login/refresh/restaurants — factorisé maintenant, comme annoncé au ticket précédent) parse l'enveloppe `{success,data}`/`{success,error}` et transmet `Accept-Language` (aligne les messages d'erreur déjà localisés côté serveur sur la langue choisie dans l'app, pas seulement celle du navigateur) ; `authorizedFetch` retente une seule fois après un rafraîchissement silencieux sur `AUTH_TOKEN_MISSING`/`AUTH_TOKEN_INVALID`, jamais en boucle. `RestaurantSettingsScreen.vue` : nom, logo (URL), coordonnées (téléphone/email/adresse/ville), horaires (7 jours, case à cocher "ouvert" + heures `open`/`close`, jour absent du payload = fermé) — fidèle à `updateRestaurantSchema`/mockup `docs/design/10-statistiques-parametres.html` (onglet "Général" uniquement ; "Langue & devise"/notifications restent hors périmètre littéral, `PATCH /restaurants/me/settings`, un ticket à la fois) ; bouton de déconnexion (`POST /auth/logout`). **Deux incohérences de mockup supplémentaires signalées, non corrigées** : (1) la case "Rester connecté" du mockup n'a aucune contrepartie dans `loginSchema` (aucun champ de ce type accepté côté serveur) — non reprise ; (2) "Changer le logo" suppose un vrai upload de fichier, mais aucune infrastructure de stockage (S3/multer/Cloudinary) n'existe côté API (`restaurants.model.ts` ne porte qu'un `logoUrl: string`) — construit comme un simple champ URL, pas un sélecteur de fichier (un vrai upload est une fonctionnalité à part entière, cf. ticket "gestion du menu avec upload photo", Feature 3.2). **Sélecteur multi-tenant hors périmètre** : `POST /auth/select-tenant`, évoqué en commentaire dans `auth.service.ts`, n'existe pas côté API — un utilisateur avec 0 ou 2+ memberships verra l'erreur `TENANT_CONTEXT_REQUIRED` (déjà localisée côté serveur) remonter telle quelle depuis `RestaurantSettingsScreen`, pas d'écran de sélection dédié ; sans impact sur le cas principal (un `restaurant_owner` fraîchement provisionné n'a qu'un seul membership). `App.vue` bascule désormais entre `LoginScreen`/`RestaurantSettingsScreen` (toujours pas de Router — une porte à deux états, pas une navigation entre pages adressables) ; `CountryDetectionScreen` (ticket précédent, flux d'inscription) reste entièrement implémenté/testé mais n'est plus monté par défaut (les deux flux ne peuvent pas coexister sans Router, hors périmètre de ce ticket). Vérifié réellement : 47 tests (dont apiClient, auth.store — login/2FA/logout/restoreSession/authorizedFetch avec rejeu sur token expiré —, LoginScreen, RestaurantSettingsScreen) tous verts ; `pnpm typecheck`/`eslint` propres (0 erreur). **Vérification navigateur partielle**, même blocage déjà connu qu'au ticket précédent : le serveur de dev API refuse toujours de démarrer (quota Upstash épuisé, fail-fast au boot, sans lien avec ce ticket) — impossible de vérifier le flux bout-en-bout avec un vrai backend actif. Vérifié à la place : le serveur de dev Vite compile et sert tous les nouveaux modules sans erreur (`main.ts` avec Pinia résolu, `LoginScreen.vue`, `RestaurantSettingsScreen.vue`, `auth.store.ts`, `apiClient.ts`) et le proxy `/api` forwarde bien vers le port 3000 (500 de connexion refusée, pas un 404). **Toujours pas repoussé sur `origin`** : quota Upstash toujours épuisé (réinitialisation 16 août).

### Feature 2.2 — Employés

🎯 **Livrable** : le gérant invite, gère et retire des employés, dans la limite du quota de son plan.

- [x] Implémenter `POST/GET/PATCH/DELETE /employees` — 1j — **gap RBAC signalé et corrigé** (décision validée avec toi) : `employees:read` (requis par `GET /employees`(`/:id`), doc 09 §9.5) n'était accordée à aucun rôle dans la matrice doc 08 §8.4 d'origine (même gap déjà signalé, non corrigé, lors du seed initial de `roleDefinitions` — sans impact tant qu'aucun endpoint `employees` n'existait). Corrigé ici, partout en cohérence : doc 08 §8.4 (nouvelle ligne `employees:read`, Owner ✅/Manager ✅ — même profil que `employees:create/update/delete`), `roleDefinitions.seed.ts` (nouvelle version seedée), et les 3 fichiers de tests qui en dépendent (transcription indépendante de la matrice, sweep RBAC réel, test d'idempotence du seed — ce dernier ne fige plus un numéro de version absolu, cette base Atlas partagée accumulant les versions au fil des tickets qui changent réellement la matrice). **Scope de `POST /employees` validé avec toi** ("créer le compte maintenant, email au ticket suivant") : crée réellement un compte `users` avec un mot de passe aléatoire inutilisable si l'email est inconnu (aucun envoi d'email, aucune activation — le ticket suivant "flux d'invitation employé" les ajoutera, probablement en réutilisant `passwordResetToken`), ou réutilise un compte existant ; l'index unique `{tenantId, userId}` déjà présent sur `memberships` (Feature 1.1) fait qu'inviter un email déjà membre remonte en 409 `EMPLOYEE_ALREADY_MEMBER` plutôt qu'une erreur Mongo brute. `EmployeesService`/`EmployeesController`/`employees.routes.ts` (nouveaux) : `GET /employees` (pagination offset `page`/`limit`, doc 09 §9.2, 1ère liste paginée du projet — `BaseRepository` gagne une méthode `count()` générique pour ça, réutilisable par les futures listes de configuration), filtrable par `role`/`status` ; `GET/PATCH/DELETE /employees/:id` (`DELETE` = désactivation du membership, `employmentStatus:'inactive'`, jamais une suppression réelle, doc 09 §9.5). `salary` (doc 05 : "visible uniquement par restaurant_owner/manager habilité", `employees:view_salary`) filtré au niveau DTO selon les permissions résolues de l'appelant — `resolvePermissions` extrait de `rbac.middleware.ts` (2ᵉ consommateur réel, même précédent que `createRedisRateLimiter`/`requireTenantContext`) pour décider sans jamais bloquer toute la requête comme le ferait `requirePermission`. `.populate('userId', ...)` (1ère utilisation du projet, `membership.model.ts` porte déjà `ref:'User'`) plutôt que deux requêtes séparées pour joindre l'identité `users`. Nouveaux codes **400 `EMPLOYEE_INVALID_PAYLOAD`**, **404 `EMPLOYEE_NOT_FOUND`**, **409 `EMPLOYEE_ALREADY_MEMBER`** (fr/en/it/es). La limite `maxEmployees` (409 `EMPLOYEE_LIMIT_REACHED`) reste le ticket suivant de cette Feature, pas anticipée (doc 14 §14.5 KISS). Vérifié réellement : 61 tests (validators, service, controller — unitaires/mockés — + 21 tests d'intégration réels contre MongoDB Atlas : création avec compte inline, réutilisation d'un compte existant, 409 doublon, 403 par rôle sur les 4 routes, 404 anti-IDOR cross-tenant sur GET/PATCH, pagination réelle, visibilité `salary` owner/manager/override) ; suite complète `apps/api` relancée, 667/667 verts (72 fichiers). **Deux bugs de collision inter-fichiers découverts et corrigés au passage** (même classe que `roleDefinitions`/`countryDefaults`, Feature 1.4/2.1, cette fois sur `users` et `helloWorld`) : `rbac.middleware.integration.spec.ts` faisait un `UserModel.collection.deleteMany({})` **sans aucun filtre** en fin de suite — découvert en re-testant manuellement dans le navigateur après la disparition du compte de test créé pour l'occasion ; `hello-world.integration.spec.ts` avait le même défaut sur `HelloWorldModel`. Les deux nettoyages déjà faits via `cleanupTenantFixtures`/des ids trackés suffisaient déjà : les `deleteMany({})` globaux étaient purement redondants et dangereux, retirés/scopés. **Poussé et vérifié en CI** : `925efa6`→ nouveaux commits, CI GitHub Actions verte (Lint/Build/Test), quota Upstash débloqué depuis la rotation de base (voir tickets précédents).
- [ ] Implémenter la limite `maxEmployees` du plan (`409`) — 0,5j — **reporté, décision validée avec toi** : nécessite `subscriptionPlans.limits.maxEmployees` (doc 05), or `subscriptionPlans`/`subscriptions` n'existent pas du tout (Feature 9.1, Epic 9, pas commencée) et aucune limite par défaut n'est documentée nulle part (contrairement aux gaps RBAC déjà rencontrés, il n'y a ici aucune valeur à transcrire, seulement à inventer). Repris quand Feature 9.1 aura construit de vrais `subscriptionPlans`.
- [x] Implémenter le flux d'invitation employé (email + activation) — 1j — réutilise intégralement le mécanisme "mot de passe oublié" déjà construit (doc 07 §7.5, `passwordResetToken`/`PASSWORD_RESET_TOKEN_TTL_MS`/`POST /auth/reset-password`) plutôt qu'un second système de token dupliqué : `AuthRepository` et les utilitaires de token sont désormais exportés depuis `modules/auth/index.ts` pour être réutilisés par `EmployeesService`. Quand `POST /employees` crée un compte inline (email inconnu), un token d'activation est généré/stocké puis un email est enfilé (BullMQ, nouveau template `employeeInvitation.template.ts`, fr/en/it/es) — **envoyé seulement après la création réussie du membership, jamais avant** (sinon un doublon détecté par l'index unique `{tenantId, userId}` enverrait quand même une invitation pour un rattachement qui a échoué). Un compte déjà existant (email déjà connu) n'a pas besoin d'activation, aucun email envoyé dans ce cas. Lien d'activation pointe vers `/activate-account` (pas `/reset-password`, pour refléter l'intention réelle côté utilisateur) mais consomme le même `POST /auth/reset-password` déjà existant côté API — aucune nouvelle route. Le template ne nomme volontairement pas le restaurant : le récupérer exigerait que `employees` dépende de `restaurants` (`RestaurantsRepository`), alors que `restaurants` dépend déjà de `employees` (`MembershipsRepository`) — dépendance circulaire entre modules pour un simple embellissement de copie, non justifiée. Vérifié réellement : 20 tests unitaires (template fr/en/it/es, service — email envoyé/pas envoyé selon compte nouveau/existant, jamais avant la création du membership) + 1 test d'intégration réel de bout en bout contre MongoDB Atlas + Redis Upstash (BullMQ) : invitation → extraction du token depuis le job email réellement enfilé → `POST /auth/reset-password` réel → connexion avec le mot de passe choisi ; suite complète `apps/api` relancée, 675/675 verts (73 fichiers).
- [x] Écran back-office : liste et gestion des employés — 1j — **3 points d'architecture validés avec toi avant codage** : (1) `apps/web` n'avait aucun Router (`App.vue` ne basculait qu'entre `LoginScreen`/`RestaurantSettingsScreen` via un simple `v-if`, doc 14 §14.5) — ce ticket est justement le point de bascule que doc 11 §11.5 anticipait (1 → 2 écrans authentifiés adressables) : `vue-router` introduit (2 routes, `/restaurant`/`/employees`, `createWebHashHistory` plutôt que `createWebHistory` — aucune règle de rewrite SPA côté hébergement, `vercel.json` ou équivalent, n'existe encore), nouveau `components/layout/AppShell.vue` (sidebar + `<RouterView>`, chaque écran garde sa propre barre supérieure/déconnexion plutôt que de la centraliser — coûte moins qu'un refactor de `RestaurantSettingsScreen.spec.ts` existant). (2) doc 36 §36.5 documente une sidebar générée depuis des permissions résolues via `usePermissions()`, mais aucun endpoint n'expose ces permissions résolues au frontend (le login ne renvoie que le rôle brut, et `tenants[]` ne survit pas à un rechargement — `POST /auth/refresh` ne renvoie qu'un `accessToken`, doc 07 §7.4) — **incohérence signalée, non corrigée** : gating de l'entrée "Employés" par rôle brut (`restaurant_owner`/`manager`, seuls rôles avec `employees:read` aujourd'hui) décodé directement du claim `role` du payload JWT courant (`auth.store.ts`, nouveau `role` computed — décodage non-vérifié, usage UI uniquement, l'API reste le vrai garde-fou), pas une vraie résolution de permissions ; ce claim survit lui au refresh (`auth.service.ts#refresh` préserve `role`), contrairement à `tenants[]`. (3) la maquette (`docs/design/08-employes-reservations.html`) montre un filtre "Invités" à 3 états, mais le backend n'a que `employmentStatus:'active'|'inactive'` (aucun statut "invited" distinct suivi nulle part) — **incohérence de maquette signalée, non corrigée** : écran construit avec 2 filtres seulement (Actifs/Inactifs). `EmployeesScreen.vue` (nouveau) : liste paginée (offset, doc 09 §9.2), filtres de statut, drawer d'invitation (nom/email/rôle/poste/salaire optionnel), drawer d'édition (poste/salaire — jamais email/rôle, non modifiables par `PATCH`), "Désactiver"/"Réactiver" par ligne (`DELETE`/`PATCH{employmentStatus}`, jamais une suppression réelle). Colonne/champ `salary` affichés uniquement si la clé est réellement présente dans la réponse API (le serveur l'omet déjà pour un acteur sans `employees:view_salary`, `salary:undefined` → absent du JSON) — pas de re-décision RBAC côté client, juste une lecture de ce que le serveur a déjà décidé. `apiClient.ts`/`auth.store.ts` étendus (`apiRequestWithMeta`/`authorizedFetchWithMeta`) pour exposer `meta` (pagination) en plus de `data` — 1ᵉʳ endpoint paginé consommé côté frontend, réutilisable par les futures listes de configuration (rooms, tables, catégories…). 4 locales (fr/en/it/es) complétées (`employees.*`, `app.nav.*`). Vérifié réellement : 64 tests `apps/web` tous verts (`EmployeesScreen` : chargement, colonne salaire présente/absente selon la réponse, changement de filtre, invitation POST, édition PATCH, désactivation DELETE + réactivation PATCH, pagination, erreur ; `AppShell` : gating par rôle owner/manager/waiter ; `auth.store` : décodage du claim `role`, `authorizedFetchWithMeta`) ; `pnpm typecheck`/`eslint` propres (mêmes warnings de style pré-existants dans tout le projet, `vue/max-attributes-per-line` etc., non introduits par ce ticket). **Vérification navigateur réelle complète** (Playwright, Chromium headless — les deux serveurs de dev, `apps/api`+`apps/web`, tournaient réellement, quota Upstash/Atlas disponible) : connexion réelle → sidebar affichant "Employés" pour le propriétaire de test → invitation d'un employé (201 réel) → édition du poste (PATCH réel) → désactivation (DELETE réel, statut passe à "Inactif") → réactivation (PATCH réel, repasse "Actif") → déconnexion, reconnexion avec un compte `waiter` de test confirmant que "Employés" est bien absent de sa sidebar. Un faux-positif de timing découvert puis écarté en cours de route (pas un bug produit) : le script de test attendait `networkidle` juste après un clic, qui peut se résoudre avant même que le `fetch` déclenché par le gestionnaire d'évènement asynchrone soit émis — remplacé par une attente déterministe sur le contenu réel du DOM, qui a confirmé que `EmployeesScreen` reflétait correctement chaque changement de statut.
