...
erDiagram
    ...
    USERS ||--o{ AUDIT_LOGS : "réalise (actorId)"
    RESTAURANTS ||--o{ AUDIT_LOGS : concerne
...
- **Relations** : parent de `memberships` (1 user → N memberships), `refreshTokens`, `auditLogs.actorId`.
...
|---|---|---|---|
| `_id` | ObjectId | — | |
| `tenantId` | ObjectId \| null | non | `null` pour les actions plateforme |
| `actorId` | ObjectId → `users` | oui | |
| `action` | string | oui | ex. `order.status_changed`, `employee.deleted` |
| `resource` | string | oui | nom de la collection concernée |
| `resourceId` | ObjectId | oui | |
| `before` / `after` | object \| null | non | diff avant/après (JSON partiel) |
| `ip` | string | non | |
| `userAgent` | string | non | |
| `createdAt` | Date | oui | |

- **Index** : `{ tenantId: 1, createdAt: -1 }` ; `{ actorId: 1, createdAt: -1 }` ; `{ resource: 1, resourceId: 1 }`.
- **Validations** : collection **append-only** — aucune route API ne permet update/delete (appliqué au niveau service, pas seulement convention).
...
| `auditLogs` | `{ tenantId: 1, createdAt: -1 }` — renommée `businessAuditLogs`, voir doc 24 |
