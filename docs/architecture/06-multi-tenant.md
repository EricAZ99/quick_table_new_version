...
2. **Plugin Mongoose `tenantScope`** appliqué à tous les schémas tenant-scoped : déclare lui-même le champ `tenantId` (requis) et son index — un schéma de module ne le redéclare jamais, il ne peut donc pas non plus l'oublier par erreur humaine — et ajoute un hook `pre('find')`, `pre('findOne')`, `pre('updateOne')`, `pre('updateMany')`, `pre('deleteOne')`, `pre('aggregate')` qui **lève une exception** si aucun `tenantId` n'est présent dans la requête au moment de l'exécution — deuxième filet de sécurité au niveau ORM, indépendant du respect du BaseRepository. Couvre explicitement les 5 méthodes exposées par `BaseRepository` (point 1 ci-dessus) — `updateOne`/`deleteOne` manquaient de la liste initiale, corrigé le 2026-07-15 (incohérence détectée en implémentant le plugin, doc 17 §17.7).
